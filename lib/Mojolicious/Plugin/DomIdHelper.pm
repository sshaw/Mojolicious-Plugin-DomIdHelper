...
DOM IDs are generated by joining a the object's package name with its PK. It's assumed that the
primary key can be retrieved via an method named C<id>. This can be modified, see L</DEFAULTS>
...
If the PK is undefined only the package name is returned.

Multi-column primary keys are not seperated by the C<delimiter> option. They are concatenated.

=head1 ORMs

Trying to be ORM agnostic. Just set the C<method> option to the method used to 
retrieve your object's PK.

Multi-column primary keys returned as array refs will cause problems (for now). 
...
Character used to delimit the object's package name from its primary key. Defaults to '_'.
...
Method used to retrieve the object's PK. Defaults to 'id'.
